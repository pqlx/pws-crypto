...
class AESState:
    ...
    def inv_sub_bytes(self):
        ...
        for i, b in enumerate(self._block):
            self._block[i] = self.inv_sbox[b]

    def mix_columns(self):
        self._mix_columns(inv=False)
    
    def inv_mix_columns(self):
        self._mix_columns(inv=True)
    


    def _mix_columns(self, inv: bool=False):
        ...
        def mix_column(column: bytes):
            ...
            i2e = lambda X: [field.element(x) for x in (X)]
            ce = lambda *X: compute_element(*i2e(X))
    ...
    def shift_rows(self):
        self._shift_rows(inv=False)

    def inv_shift_rows(self):
        self._shift_rows(inv=True)

    def _shift_rows(self, inv: bool=False):
        """
        ...
        for i in range(1, 4):
            ...
            self[0, i], self[1, i], \
            self[2, i], self[3, i] = (
                    self[((0 + a*i) % 4), i], self[((1 + a*i) % 4), i],
                    self[((2 + a*i) % 4), i], self[((3 + a*i) % 4), i]
                    )
